...
fn read_wing_data(image: &RgbaImage) -> Option<WingData> {
    ...
    Some(WingData { mode, animated })
}

fn read_snout_data(image: &RgbaImage) -> Option<SnoutData> {
...
fn read_tail_data(image: &RgbaImage) -> Option<TailData> {
    ...
    let tail_bend1 = px_val_to_unit((tail_bend & 0x00FF0000) >> 16) * 90.0;
    let tail_bend2 = px_val_to_unit((tail_bend & 0x0000FF00) >> 8) * 90.0;
    let tail_bend3 = px_val_to_unit(tail_bend & 0x000000FF) * 90.0;
...
mod tests {
    ...
    fn v0_ear_mode_none() {
        let image = image::open("test_images/ears_v0_sample1.png").unwrap();
        let image = image.to_rgba8();
        let features = EarsParserV0::parse(&image).unwrap();
